...
def test_load_data():
    ...
    assert historical_weather.percentage_snowfall() == 23 / 34

    assert historical_weather.contiguous_precipitation() == (date(2020, 4, 8), 9)


def test_more_load_data():
    """Test load_data on bigger dataset."""
    with open('student_data/en_climate_daily_ON_6110480_2020_P1D (2).csv') as source:
        historical_weather = load_data(source)

    assert historical_weather is not None

    assert historical_weather.name == 'BALDWIN'

    assert round(historical_weather.percentage_snowfall(), 2) == 0.21

    assert historical_weather.contiguous_precipitation() == (date(2020, 12, 22), 9)
